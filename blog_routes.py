...
@login_required
def minha_lista():
    salvos = (BlogPost.query
              .join(BlogSalvo, BlogSalvo.post_id == BlogPost.id)
              .filter(BlogSalvo.user_id == current_user.id,
                      BlogPost.status == 'publicado')
              .order_by(BlogSalvo.id.desc())
              .all())
    curtidos = (BlogPost.query
                .join(BlogCurtida, BlogCurtida.post_id == BlogPost.id)
                .filter(BlogCurtida.user_id == current_user.id,
                        BlogPost.status == 'publicado')
                .order_by(BlogCurtida.id.desc())
                .all())
    salvos_ids   = {p.id for p in salvos}
    curtidos_ids = {p.id for p in curtidos}
    return render_template('blog/minha_lista.html',
                           salvos=salvos,
                           curtidos=curtidos,
                           salvos_ids=salvos_ids,
...
def admin_categorias():
    if request.method == 'POST':
        ...
        if BlogCategoria.query.filter_by(nome=nome).first():
            flash(f'Já existe uma categoria com o nome "{nome}".', 'error')
            return redirect(url_for('blog.admin_categorias'))
...
def admin_newsletter():
    inscritos = BlogNewsletter.query.order_by(BlogNewsletter.created_at.desc()).all()
...
def admin_newsletter_excluir(id):
    inscrito = BlogNewsletter.query.get_or_404(id)
    db.session.delete(inscrito)
    db.session.commit()
    flash(f'E-mail "{inscrito.email}" removido da newsletter.', 'success')
...
def admin_newsletter_exportar():
    ...
    def hex_to_rgb(h):
        h = (h or '#5B6D3D').lstrip('#')
        return tuple(int(h[i:i+2], 16) for i in (0, 2, 4))

    pr = hex_to_rgb(cfg.color_primary   if cfg else None)
    sc = hex_to_rgb(cfg.color_secondary if cfg else '#D67155')
    ...
    pdf.set_y(8)
    pdf.cell(0, 10, 'Lista de Inscritos - Newsletter', align='C', ln=True)
    pdf.set_font('Helvetica', '', 9)
    pdf.cell(0, 6, f'Exportado em {datetime.datetime.now().strftime("%d/%m/%Y as %H:%M")}  |  Total: {len(inscritos)} inscrito(s)', align='C', ln=True)

    pdf.set_y(36)
    pdf.set_text_color(30, 41, 59)

    # Cabeçalho da tabela com cor secundária suave
    r2, g2, b2 = sc
    pdf.set_fill_color(min(r2+160, 255), min(g2+160, 255), min(b2+160, 255))
    pdf.set_font('Helvetica', 'B', 9)
    pdf.set_draw_color(226, 232, 240)
    pdf.cell(10,  9, '#',              border=1, fill=True, align='C')
    pdf.cell(115, 9, 'E-mail',         border=1, fill=True)
    pdf.cell(55,  9, 'Data inscricao', border=1, fill=True, ln=True)

    # Linhas com zebra usando tom da cor primária
    r1, g1, b1 = pr
    pdf.set_font('Helvetica', '', 8.5)
    for idx, i in enumerate(inscritos, 1):
        if idx % 2 == 0:
            pdf.set_fill_color(min(r1+175, 255), min(g1+175, 255), min(b1+175, 255))
        else:
            pdf.set_fill_color(255, 255, 255)
        pdf.cell(10,  8, str(idx),                                   border=1, fill=True, align='C')
        pdf.cell(115, 8, i.email,                                    border=1, fill=True)
        pdf.cell(55,  8, i.created_at.strftime('%d/%m/%Y  %H:%M'),  border=1, fill=True, ln=True)
