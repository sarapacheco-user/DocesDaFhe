    ...
    if request.method == 'POST':
        ...
        def validate_cep_exists(cep):
            ...
            try:
                ...
                app.logger.error(f"[CEP] Erro ao validar CEP {cep}: {type(e).__name__}: {e}")
                return False

        old_data = dict(name=name, email=email, phone=phone, cep=cep)

        if not name.strip():
            flash("O nome é obrigatório.", 'error')
            return render_template('auth/signup.html', old=old_data)
        ...
        if len(cep) != 8 or not cep.isdigit():
            flash("Formato de CEP inválido (deve ter 8 dígitos).", 'error')
            return render_template('auth/signup.html', old=old_data)
        if not validate_cep_exists(cep):
            flash("CEP inválido - endereço não encontrado.", 'error')
            return render_template('auth/signup.html', old=old_data)
        if not validate_phone(phone):
            flash("Número de telefone inválido (deve ter 10-11 dígitos, com DDD).", 'error')
            return render_template('auth/signup.html', old=old_data)
